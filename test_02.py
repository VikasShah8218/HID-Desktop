...
class HIDSimulator(QWidget):
    ...
    def init_ui(self):
        ...
        for i in range(6):  # 6 devices for example
            device_frame = QFrame()
            device_frame.setFrameShape(QFrame.Shape.Box) # div
            device_frame.setLineWidth(2)     # border width 2px
            device_frame.setFixedSize(250, 150) # w=250 h =150
            ...
            controller_name.setAlignment(Qt.AlignmentFlag.AlignLeft)

            # Live status icons
            live_status = QLabel('Controller is Live ✅')
            live_status.setAlignment(Qt.AlignmentFlag.AlignLeft)
            driver_status = QLabel('Driver is Live ✅')
            driver_status.setAlignment(Qt.AlignmentFlag.AlignLeft)
        ...
        log_box = QTextEdit()
        log_box.setPlaceholderText("Log details will be displayed here...")
        log_group_layout = QVBoxLayout()
        log_group_layout.addWidget(log_box)
